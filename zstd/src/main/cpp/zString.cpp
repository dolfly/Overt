...
// Compound assignment operator (+=) for char
    ...
    size_t string::find_last_of(const char* str, size_t pos) const {
        // Log the call
        LOGV("nonstd::string.find_last_of(char*) called with '%s' at pos %zu", str, pos);
        ...
        for (int i = static_cast<int>(pos); i >= 0; --i) {
            for (size_t j = 0; j < str_len; ++j) {
                if (data[i] == str[j]) {
                    LOGV("nonstd::string.find_last_of(char*): found '%c' at position %zu", str[j], static_cast<size_t>(i));
                    return static_cast<size_t>(i);
                }
            }
        }
