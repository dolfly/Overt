            ...
            if (string_end_with(it->first.c_str(), lib_name.c_str())) {
                ...
                if (it->second.size() != 4) {
                    ...
                    // 检查权限是否正确（正常情况下应该是r--p, r-xp, r--p, rw-p）
                         ...
                         (it->second[1].permissions != "r-xp" && it->second[1].permissions != "--xp") ||
                         (it->second[2].permissions != "r--p" && it->second[2].permissions != "rw-p") ||
                         (it->second[3].permissions != "rw-p" && it->second[3].permissions != "r--p"))) {
