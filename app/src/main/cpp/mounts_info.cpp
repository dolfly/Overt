...
std::map<std::string, std::string> get_mounts_info(){
    ...
    for(int i = 0; i < mounts_lines.size(); i++){
        for (const char* path : paths) {
            if (strstr(mounts_lines[i].c_str(), path) != 0) {
                LOGE("check_mounts error %d %s", i, mounts_lines[i].c_str());
                info[mounts_lines[i].c_str()] = "error";
            }else if(strstr(mounts_lines[i].c_str(), "/system ") != 0 && strstr(mounts_lines[i].c_str(), "overlay") != 0){
                LOGE("check_mounts error %d %s", i, mounts_lines[i].c_str());
                info[mounts_lines[i].c_str()] = "error";
            }
        }
