...
static int find_root_of_trust_in_authorization_list(const unsigned char* data, int len, root_of_trust_t* root_of_trust);
static int parse_root_of_trust_sequence(const unsigned char* data, int len, root_of_trust_t* root_of_trust);
static void print_hex_data(const char* prefix, const unsigned char* data, int len, const char* suffix);

// Get extension by OID using manual ASN.1 parsing (for debugging OID comparison)
static int get_extension_by_oid_manual(const unsigned char* data, int len, const char* oid_str, unsigned char** ext_data, int* ext_len) {
    *ext_data = NULL;
    *ext_len = 0;
    ...
    while (offset < len) {
        LOGE("[Native-TEE] get_extension_by_oid_manual: parsing at offset %d, remaining len %d", offset, len - offset);
        
        if (parse_asn1_tag(data, len, &offset, &obj) != 0) {
            LOGE("[Native-TEE] get_extension_by_oid_manual: failed to parse tag at offset %d", offset);
            break;
        }
        ...
        if ((obj.tag & 0x1F) == ASN1_SEQUENCE && obj.is_constructed) {
            ...
            while (ext_offset < obj.data_len) {
                LOGE("[Native-TEE] get_extension_by_oid_manual: parsing extension at offset %d, remaining %d", ext_offset, obj.data_len - ext_offset);
                ...
                LOGE("[Native-TEE] Extension sequence: tag=0x%02x, length=%d", extension_sequence.tag, extension_sequence.length);
                
                // Parse extension ID (OID) - first element in sequence
                int seq_offset = 0;
                if (parse_asn1_tag(extension_sequence.data, extension_sequence.data_len, &seq_offset, &ext_obj) != 0) {
                    LOGE("[Native-TEE] get_extension_by_oid_manual: failed to parse extension OID");
                    ext_offset += extension_sequence.length;
                    continue;
                }
            
            LOGE("[Native-TEE] Extension OID: tag=0x%02x, length=%d", ext_obj.tag, ext_obj.length);
            ...
            if (ext_obj.tag == ASN1_OBJECT_IDENTIFIER) {
                ...
                if (ext_obj.data_len == 10) {
                    if (ext_obj.data[0] == 0x2B && ext_obj.data[1] == 0x06 && ext_obj.data[2] == 0x01 && 
                        ext_obj.data[3] == 0x04 && ext_obj.data[4] == 0x01 && ext_obj.data[5] == 0xD6 && 
                        ext_obj.data[6] == 0x79 && ext_obj.data[7] == 0x02 && ext_obj.data[8] == 0x01 && 
                        ext_obj.data[9] == 0x11) {
                        ...
                        if (seq_offset < extension_sequence.data_len) {
                            ...
                            if (parse_asn1_tag(extension_sequence.data, extension_sequence.data_len, &seq_offset, &critical_obj) == 0) {
                                if (critical_obj.tag == ASN1_BOOLEAN) {
                                    LOGE("[Native-TEE] Extension critical flag: %d", critical_obj.data[0]);
                                    // Continue to parse the extension value (third element)
                                } else {
                                    // This is the extension value, not critical flag
                                    LOGE("[Native-TEE] No critical flag, this is the extension value: tag=0x%02x", critical_obj.tag);
                                    if (critical_obj.tag == ASN1_OCTET_STRING_TAG) {
                                        LOGE("[Native-TEE] Found attestation extension value: length=%d", critical_obj.data_len);
                                        *ext_data = (unsigned char*)critical_obj.data;
                                        *ext_len = critical_obj.data_len;
                                        return 0;  // SUCCESS - found the extension!
                                    }
                                    // If we get here, something went wrong
                                    LOGE("[Native-TEE] Expected OCTET_STRING for extension value, got tag=0x%02x", critical_obj.tag);
                                    return -1;
                                }
                            }
                        }
                        ...
                        if (seq_offset < extension_sequence.data_len) {
                            if (parse_asn1_tag(extension_sequence.data, extension_sequence.data_len, &seq_offset, &ext_obj) == 0) {
                                if (ext_obj.tag == ASN1_OCTET_STRING_TAG) {
                                    LOGE("[Native-TEE] Found attestation extension value: length=%d", ext_obj.data_len);
                                    *ext_data = (unsigned char*)ext_obj.data;
                                    *ext_len = ext_obj.data_len;
                                    return 0;  // SUCCESS - found the extension!
                                } else {
                                    LOGE("[Native-TEE] Expected OCTET_STRING for extension value, got tag=0x%02x", ext_obj.tag);
                                    return -1;
                                }
                            }
                        }
                        ...
                        LOGE("[Native-TEE] Failed to parse extension value for OID %s", oid_str);
                        return -1;
                    }
                }
            }
        ...
        LOGE("[Native-TEE] get_extension_by_oid_manual: skipping non-SEQUENCE object (tag=0x%02x)", obj.tag);
    }
        
        // Move to next extension in the outer sequence
        LOGE("[Native-TEE] get_extension_by_oid_manual: moving to next extension, current offset=%d, obj.length=%d", offset, obj.length);
        offset += obj.length;
        LOGE("[Native-TEE] get_extension_by_oid_manual: new offset=%d, remaining len=%d", offset, len - offset);
    }
...
static void print_hex_data(const char* prefix, const unsigned char* data, int len, const char* suffix) {
    ...
    for (int line = 0; line < lines; line++) {
        ...
        // Print line header
        pos += snprintf(hex_buf + pos, sizeof(hex_buf) - pos, "[Native-TEE] %s[%d-%d]: ", prefix, start, end - 1);
...
static int parse_x509_certificate(const unsigned char* data, int len, tee_info_t* tee_info) {
    ...
    if (body_len > 0) {
        ...
        if (tag == ASN1_CONSTRUCTED_SEQUENCE || tag == 0xa3) {
            ...
            if (get_extension_by_oid_manual(ext_data, ext_len, TEE_ATTESTATION_OID, &extension_data, &extension_len) == 0) {
...
static int parse_attestation_record(const unsigned char* data, int len, tee_info_t* tee_info) {
    ...
    if ((att_obj.tag & 0x1F) == ASN1_SEQUENCE && att_obj.is_constructed) {
        ...
        find_root_of_trust_in_authorization_list(att_obj.data, att_obj.data_len, &tee_info->root_of_trust);
    }
...
static int parse_root_of_trust_sequence(const unsigned char* data, int len, root_of_trust_t* root_of_trust) {
    ...
    LOGE("[Native-TEE] Root of trust data: tag=0x%02x, length=%d, constructed=%d", obj.tag, obj.length, obj.is_constructed);
