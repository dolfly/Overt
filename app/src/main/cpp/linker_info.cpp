...
map<string, map<string, string>> get_linker_info(){
    ...
    for (int i = 0; i < libpath_list.size(); ++i) {
        LOGD("[linker_info] libpath %s", libpath_list[i].c_str());
        if(strstr(libpath_list[i].c_str(), "lsposed")){
            LOGW("[linker_info] Found blacklisted library: %s", libpath_list[i].c_str());
    ...
    for(int i = 0; i < so_list.size(); ++i) {
        ...
        if(string_end_with(so_name.c_str(), ".so")){
            ...
            LOGD("[linker_info] check_lib_crc %s %d", so_name.c_str(), ret);
            if (ret!= 0){
                LOGW("[linker_info] CRC check failed for library: %s", so_name.c_str());
